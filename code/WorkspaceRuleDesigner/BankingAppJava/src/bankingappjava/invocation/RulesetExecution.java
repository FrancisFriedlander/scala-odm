...
abstract class RulesetExecution {
	...
	private static final String RULE_APP_DIRECTORY = "../../shared";
	...
	public void run() throws RulesetExecutionException {
		...
		try {
			...
			String rulesetName = rulesetPath.substring(rulesetPath
					.lastIndexOf("/"));
	...
	private synchronized IlrJ2SESessionFactory getFactory() {
		if (sessionFactory == null) {
			IlrSessionFactoryConfig config = IlrJ2SESessionFactory
					.createDefaultConfig();
			config.getXUConfig().getPersistenceConfig()
					.setPersistenceType(IlrPersistenceType.FILE);
			config.getXUConfig().getPersistenceConfig()
